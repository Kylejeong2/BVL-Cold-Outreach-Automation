...
export class ApplicationAnalyzer {
  ...
  async analyzeApplication(application: ApplicationResponse): Promise<{
    ...
    const finalStatus = this.determineFinalStatus(firstAnalysis, secondAnalysis);
    const needsManualReview = this.checkIfNeedsReview(firstAnalysis, secondAnalysis);
  ...
  private async runAnalysis(application: ApplicationResponse): Promise<AnalysisResult> {
